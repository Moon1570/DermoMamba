"""
Comprehensive validation script to test all checkpoints and calculate Dice & IoU scores
"""
import os
import torch
...
class CheckpointValidator:
    def __init__(self, data_path, image_size=224, batch_size=8):
    ...
    def setup_validation_data(self):
        """Setup validation dataset"""
        print(f"Setting up validation data from: {self.data_path}")
        ...
        try:
            # Try loading with splits file
            splits_file = 'd:/Research/DermoMamba/splits/isic2018_val.txt'
            if os.path.exists(splits_file):
                with open(splits_file, 'r') as f:
                    val_files = [line.strip() for line in f.readlines()]
                print(f"Found {len(val_files)} validation files in splits")
            else:
                # Fallback: use a subset of available files
                val_files = None
                print("No splits file found, will use subset of data")
                
        except Exception as e:
            print(f"Warning loading splits: {e}")
            val_files = None
        
        # Create datasets for both 3-channel and 6-channel models
        try:
            # Define split file path
            splits_file = 'd:/Research/DermoMamba/splits/isic2018_val.txt'
            
            # 3-channel dataset
            self.val_dataset_3ch = ISICDataset(
            ...
            # 6-channel dataset  
            self.val_dataset_6ch = ISICDataset(
                data_root=self.data_path,
                split_file=splits_file,
        ...
        try:
            ...
            for key in state_dict.keys():
                if 'patch_embed' in key and 'weight' in key:
                    ...
                    first_layer_key = key
                    break
                elif 'conv1' in key and 'weight' in key:
                    first_layer_key = key
                    break
    ...
    def print_summary(self, results):
        ...
        print(f"{'Rank':<4} {'Checkpoint':<40} {'Model':<15} {'Ch':<3} {'Dice %':<8} {'IoU %':<8}")
        print("-" * 80)
        
        for i, result in enumerate(results_sorted, 1):
            checkpoint_name = os.path.basename(result['checkpoint_path'])
            ...
            print(f"{i:<4} {checkpoint_name:<40} {model_type:<15} {channels:<3} {dice_pct:<8.2f} {iou_pct:<8.2f}")
...
def main():
    print("Starting comprehensive checkpoint validation...")
    ...
    validator = CheckpointValidator(data_path=data_path)
